...
for a given RPC. Applications should only use values defined below. gRPC libraries
...
See the [Error handling](https://github.com/grpc/grpc.io/blob/main/content/en/docs/guides/error.md) user guide
...
### Codes returned by the gRPC libraries
...
Applications that may wish to [retry](https://github.com/grpc/proposal/blob/master/A6-client-retries.md)
failed RPCs must decide upon which status codes they will retry. As shown in the table
below, the gRPC library can generate the same status code for different cases.
Server applications can also return those same status codes. Therefore, there is
no fixed list of status codes on which it is appropriate to retry in all
applications. As a result, individual applications must make their own determination
as to which status codes should cause an RPC to be retried.
